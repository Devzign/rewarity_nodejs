...
// POST /api/auth/login
// OTP-only flow
// - Send OTP: Body { mobile }
// - Verify OTP: Body { mobile, code }
async function login(req, res) {
  try {
    ...
    // If no code yet, issue OTP and return
    if (!code) {
...
async function generateUniqueUserCode(typeName) {
  // Dealer/Salesperson: 16-digit with prefix; Distributor: 6-digit 100000-999999
  const MAX_TRIES = 10;
  let attempt = 0;
...
// Body: { userName, primaryMobile, typeName, email?, address1?, address2?, cityName? }
...
// POST /api/auth/request-otp  Body: { mobile, purpose }
...
// POST /api/auth/verify-otp  Body: { mobile, code }
